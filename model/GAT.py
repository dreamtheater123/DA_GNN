...
class GAT(torch.nn.Module):
    def __init__(self, config):
        ...
        self.conv1 = GATConv(config.input_layer, config.hidden_layer_GAT, heads=config.num_heads, dropout=config.dropout)
    ...
    def forward(self, this_data, config):
        x = F.dropout(this_data.x, p=config.dropout, training=self.training)
